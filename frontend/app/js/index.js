...
function protectedRoute() {
    ...
    .then((data) => {
        if (data.error) {
            console.log("Error while fetching protected route: " + data.error);
    ...
    .catch((error) => {
        console.log("Error while fetching protected route: " + error);
        window.location.href = "http://localhost:8181/login.html";
    });
}
...
async function getAuthMethods() {
    try {
        const response = await fetch("http://localhost:3002/auth-method", {
            method: "GET",
            credentials: "include",
        });
